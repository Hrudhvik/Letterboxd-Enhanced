...
| **OMDb** | [omdbapi.com/apikey.aspx](https://www.omdbapi.com/apikey.aspx) | Optional | Optional fallback for some missing Rotten Tomatoes + Metacritic ratings. |
| **Jikan** | — | No key needed | MyAnimeList ratings (free public API) |

> **TMDB is the primary source.** It provides excellent international coverage and returns the IMDb ID for each film. We then use direct scraping to fetch live IMDb, Rotten Tomatoes, and Metacritic ratings. OMDb serves only as an optional fallback.
